  ...
  return (
    <div className="bg-white shadow-sm border-b border-gray-200 p-2 sm:p-3 md:p-4">
      <div className="flex flex-col gap-2 sm:gap-3 md:gap-4">
        {/* Top row: Date navigation - Mobile optimized */}
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            onClick={goToToday}
            className="btn-secondary text-xs sm:text-sm px-2 sm:px-3 py-1.5 sm:py-2 whitespace-nowrap flex-shrink-0"
          >
            {t('calendar.today')}
          </button>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-1">
            ...
            <div className="relative flex items-center flex-1 min-w-0">
              <input
                ...
                readOnly
                className="px-2 sm:px-3 py-1.5 sm:py-2 pr-7 sm:pr-8 md:pr-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent w-full max-w-28 sm:max-w-32 text-xs sm:text-sm cursor-pointer pointer-events-none"
              />
              <input
              ...
              />
              <svg
                className="absolute right-1.5 sm:right-2 md:right-3 w-3 h-3 sm:w-4 sm:h-4 text-gray-400 pointer-events-none z-10"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          {/* Date display - hidden on mobile, shown on sm+ */}
          <h2 className="hidden sm:block text-sm sm:text-base md:text-lg lg:text-xl font-semibold text-gray-800 flex-1 text-center sm:text-left truncate">
            {formatDateDisplay(currentDate, t)}
          </h2>
        </div>

        {/* Bottom row: View switcher - Mobile optimized */}
        <div className="flex items-center gap-1 sm:gap-2 bg-gray-100 rounded-lg p-0.5 sm:p-1">
          <button
            ...
            className={`flex-1 sm:flex-initial px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-medium transition-all ${
              currentView === 'day'
        ...
        {/* Mobile date display */}
        <h2 className="text-xs sm:text-sm font-semibold text-gray-800 sm:hidden text-center">
          {formatDateDisplay(currentDate, t)}
        </h2>
      </div>
    </div>
